...
class ConsultMeteo(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/':
            self.show_html('demo_py.html')  # Affiche le formulaire de saisie
        #elif self.path.startswith('/resultat'):
            #self.show_result(self.path)  # Affiche la page de résultats avec les données météo
        else:
    ...
    def get_parameter(self):
        try:
            ...
            if "error" in response:  # Si une erreur est retournée par get_meteo
